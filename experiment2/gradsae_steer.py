...
def sae_forward_hook_factory(switch, sae, record_dict, original, steer):
...
def run_generate(messages, model, layer, switch, sae, original, steer):
    model.reset_hooks()
    record_dict = {}
    hook_fn = sae_forward_hook_factory(switch=switch, sae=sae, record_dict=record_dict, original=original, steer=steer)
...
def main(question, model, layer, switch, sae, original, steer):
    answer, output_json = run_generate(question, model, layer, switch, sae, original, steer)
    torch.cuda.empty_cache()

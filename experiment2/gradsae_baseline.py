    ...
    with model.hooks(fwd_hooks=fwd_hooks):
        ...
        for step in range(max_new_tokens):
            ...
            # if record_dict and "feature_acts" in record_dict:
                column_mean_grads = record_dict["feature_acts"].mean(dim=1).squeeze(0)
                ...
                column_mean_grads_np = column_mean_grads.detach().cpu().numpy()
                non_zero_indices = np.nonzero(column_mean_grads_np)[0]
                non_zero_sorted_indices = non_zero_indices[np.argsort(column_mean_grads_np[non_zero_indices])]
                print(len(non_zero_sorted_indices), len(sorted_indices))

                top_indices = sorted_indices[:top_k]
                bottom_indices = non_zero_sorted_indices[:top_k]

                top_dict = {int(idx): float(column_mean_grads_np[idx]) for idx in top_indices}
                bottom_dict = {int(idx): float(column_mean_grads_np[idx]) for idx in bottom_indices}
                ...
                output_json[prompt_batch[0]] = {
                    ...
                    "have_gradient": top_dict,
                    "no_gradient": bottom_dict
                }
